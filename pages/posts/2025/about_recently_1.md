...
尘埃落定了, **的`Intel`挤牙膏就算了, CPU还缩缸, 小米也是, 现在Intel都给BIOS的更新补丁了, 泥小米就是不给BIOS打补丁, 算了, 这里给出所有症状

 - 浏览器浏览网页有概率崩溃, 报错代码是什么忘记了, 不过会提示`此页存在问题`, 而且频繁发生

 - 还有就是电脑会随机蓝瓶重启, 就算什么都没干, 而且`minidump`文件会说是内存问题(我一开始也被误导了, 但本质就是CPU问题, 非法内存访问是CPU中内存转换模块出的问题, 具体原理学完CSAPP就知道了)

目前这就是所有的症状, 解决方法很简单, 不需要什么CPU降压(我试过了, 根本没用), 下载XTU然后把能效核降低3x, 我自己还降低了2x的大核心, 目前就再也没问题了.

最后, 以后再也不买Intel的CPU了, AMD YES!
